...
*   **Stick on Scroll → Corner Mini-Player:** Optionally, as you scroll the player out of view, the video shrinks into a small floating mini-player in a corner so you can keep watching while you browse. The mini is **draggable** (it snaps to and remembers the nearest corner) and **resizable** (drag the inner-corner grabber to set your preferred size). Works in default and theater view.
...
3.  Click the extension's toolbar icon to open the settings popup, where you can:
    ...
    *   Enable **Stick on Scroll** — the player shrinks into a draggable, resizable corner mini-player as you scroll. Drag it to reposition (it snaps to the nearest corner) and drag its inner-corner grabber to resize.
